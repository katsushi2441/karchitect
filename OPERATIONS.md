...
`app/policies.py` は、AIのトークン消費・計算資源を削減する目的を含むプロジェクトで、
実行時のAI / LLM / VLM利用をコードで検出する。ローカルモデルもAI利用として扱う。
検出結果はAPIの `design_warnings`、Web画面、設計書へ表示し、Gemmaへ渡す最優先制約にも
利用する。判定はLLMへ委ねず、通常プログラム、固定ルール、担当者確認を優先する。

AI利用案があるのに実行時AIの方針が未確定の場合、`policy_question` で利用者へ先に
「使わない / 利用を許可」を確認し、回答までは設計を進めない。回答はLLMを呼ばず
Pythonで `decisions` と `constraints` へ保存する。「使わない」場合は、ローカルモデルを
含むAI処理を除去し、固定ルールと担当者確認へ変更する。

有料外部APIはAI利用とは別の方針として扱う。Google Street View API等の候補がある場合、
実行時AIの確認後に「有料外部APIを使うか」を確認する。「使わない」場合は公開データ取込
または担当者入力へ変更する。
