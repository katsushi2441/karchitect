...
from .config import DATA_DIR, DEFAULT_MODEL, STATIC_DIR
from .db import (
    add_message,
    create_project,
    get_messages,
...
def _detail(project_id: str) -> ProjectDetail:
...
def projects() -> list[ProjectSummary]:
    return [_row_summary(row) for row in list_projects()]


@app.post("/api/projects", response_model=ProjectDetail)
def new_project(payload: ProjectCreate) -> ProjectDetail:
    row = create_project(payload)
    req = parse_requirements(row)
    document = build_markdown(req)
    save_project(row["id"], req, document)
    add_message(row["id"], "assistant", bootstrap_message(req))
    return _detail(row["id"])


@app.get("/api/projects/{project_id}", response_model=ProjectDetail)
def project(project_id: str) -> ProjectDetail:
    return _detail(project_id)


@app.put("/api/projects/{project_id}/requirements", response_model=ProjectDetail)
def replace_requirements(project_id: str, requirements: Requirements) -> ProjectDetail:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
    requirements.revision = max(requirements.revision, parse_requirements(row).revision + 1)
    document = build_markdown(requirements)
    save_project(project_id, requirements, document)
    add_message(project_id, "system", "要件JSONが手動更新されました。")
    return _detail(project_id)


@app.post("/api/projects/{project_id}/messages", response_model=ProjectDetail)
async def send_message(project_id: str, payload: MessageCreate) -> ProjectDetail:
    lock = project_locks.setdefault(project_id, asyncio.Lock())
    async with lock:
        row = get_project(project_id)
        if not row:
            raise HTTPException(status_code=404, detail="Project not found")
        req = parse_requirements(row)
        history_models = get_messages(project_id, limit=30)
        history = [{"role": message.role, "content": message.content} for message in history_models]
        add_message(project_id, "user", payload.content.strip())
        warning = ""
        try:
        ...
        save_project(project_id, turn.requirements, document, llm_warning=warning)
        add_message(project_id, "assistant", turn.assistant_message)
        return _detail(project_id)


@app.post("/api/projects/{project_id}/regenerate", response_model=ProjectDetail)
def regenerate(project_id: str) -> ProjectDetail:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
    req = parse_requirements(row)
    save_project(project_id, req, build_markdown(req), llm_warning=row["llm_warning"])
    return _detail(project_id)


@app.get("/api/projects/{project_id}/document.md", response_class=PlainTextResponse)
def document_markdown(project_id: str) -> PlainTextResponse:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
...
def document_json(project_id: str) -> JSONResponse:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
    req = parse_requirements(row)
...
def document_html(project_id: str) -> HTMLResponse:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
...
def document_pdf(project_id: str) -> FileResponse:
    row = get_project(project_id)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
...
def mermaid_source(project_id: str, diagram: str) -> PlainTextResponse:
